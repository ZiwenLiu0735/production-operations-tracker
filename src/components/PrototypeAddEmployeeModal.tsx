    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-6">
      <div className="flex max-h-[85dvh] w-full max-w-lg flex-col rounded-2xl border border-surface-600 bg-surface-800 shadow-2xl">
        <div className="border-b border-surface-600/50 p-6">
          <h2 className="text-xl font-bold">Add Operator</h2>
          <p className="mt-1 text-sm text-white/50">Select an operator to add to this session</p>
          <input
            type="search"
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {filteredEmployees.length === 0 ? (
            <p className="text-center text-sm text-white/40">No available operators</p>
          ) : (
