...
export function RecentEntries({ entries, compact = false, headerAction }: RecentEntriesProps) {
  if (entries.length === 0) {
    return (
      <div
  ...
  return (
    ...
    >
      <div
        className={`flex shrink-0 items-center justify-between gap-2 ${compact ? "mb-1" : "mb-2"}`}
      >
        <h3
          className={`font-semibold uppercase tracking-widest text-white/40 ${compact ? "text-[10px]" : "text-sm"}`}
        >
